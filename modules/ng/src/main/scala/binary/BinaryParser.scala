...
object BinaryParser {
	...
	def anyByteOf(s:ByteString):BinaryParser[Byte]				= Parser isInSet	s.toSet				named s"any byte in ${s.toString}"
	def anyByteInRange(from:Byte, to:Byte):BinaryParser[Byte]	= Parser isInRange	(from, to)			named s"any byte between ${from.toString} and ${to.toString}"
