...
object TextParser {
	...
	def anyCharOf(s:String):TextParser[Char]				= Parser isInSet	s.toSet				named s"any char in ${s}"
	def anyCharInRange(from:Char, to:Char):TextParser[Char]	= Parser isInRange	(from, to)			named s"any char between ${from.toString} and ${to.toString}"
